...
8. **Performance & Rendering**: Use `RepaintBoundary` and isolate rebuilds via `Selector` for heavy drawing/custom paint components (like the chess board background `_ChessBackground`, main menu backgrounds, and settings page dot grids or radial blurs). For board rendering in `ChessGame`, perform a single board-level counter-rotation Canvas operation to draw pieces upright instead of applying costly per-piece translate/rotate cycles.
9. **AI Search & GC Pressure**: Re-use Transposition Table structures during Minimax evaluations rather than re-allocating them on every search. Use `softClear()` to invalidate stale entries without releasing/reallocating the backing lists. Maintain the Transposition Table size at `1 << 18` (256k entries) to ensure high hit rates under depth 4-5 evaluations. Avoid temporary allocations in search paths: use parallel lists with in-place index sorting in `allMoves` instead of wrapper classes, check checks/attacks using direct zero-allocation methods (`_pieceAttacksTile`), and avoid snapshotting openings unless on the human move path (`getMeta` is true).
