...
- `lib/logic/`: Core game mechanics. Includes `chess_board.dart`, `chess_game.dart`, `game_controller.dart`, `move_calculation/`, `timer_service.dart`, `audio_service.dart`, `game_state_storage.dart`, and Flame Sprite logic (`chess_piece_sprite.dart`). Check Minimax and Alpha-Beta logic in `move_calculation/`.
...
5. **No Destructive Overrides**: Do not remove core AI features (Undo/Redo, Timers, Difficulty Levels) unless explicitly requested. Currently supports 5 difficulty levels.
