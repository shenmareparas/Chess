...
- **`main_menu_view.dart`**: Game configuration (1P/2P, difficulty, time control, side selection). In 2P mode Player 1 can set their starting side (`selectedSideP1`). Shows "Resume" button if `GameStateStorage.hasSavedGame()` is true. Options list and bottom buttons are max-width constrained (600px) for wide/unfolded displays.
