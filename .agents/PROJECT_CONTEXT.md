...
- **`lib/main.dart`**: Initializes preferences, then calls `runApp()` immediately to show the splash logo on the first frame. Decodes essential piece images for only active and Classic themes asynchronously after `runApp()`, signaling `appModel.imagesReady` when complete. Eliminates full background theme preloading to protect main-thread RAM and prevent `OutOfMemoryError` failures. Other piece themes load lazily on-demand when selected or previewed. Initializes AdMob in the background. Calls `PlayGamesService.instance.signInSilently()` in a non-blocking `Future.microtask()` after initial render to avoid IPC binder ANRs. Calls `InAppUpdateService.instance.checkForUpdate()` (non-blocking).
