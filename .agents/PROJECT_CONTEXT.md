...
- **`game_controller.dart`**: The logic ViewModel. Orchestrates move execution, AI triggering, undo/redo, and pawn promotion. Offloads CPU-heavy checkmate verification (`kingInCheckmate()`) to `CheckmateWorker` in a background isolate to keep UI frame times smooth. Gates AI moves with `!undoing` to resume play when undoing back onto the AI's turn. Reconstructs a clean controller on new/restored games, calling `dispose()` to tear down the background worker.
