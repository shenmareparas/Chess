...
9. **Stockfish Lifecycle**: Initialize on demand, dispose in `AppModel.dispose()`. Perform the UCI handshake (`uci` + `isready` -> `readyok`) exactly once per engine instance using a shared `_uciReadyCompleter`. Use `CancelableOperation` to cancel in-flight AI requests on game reset.
