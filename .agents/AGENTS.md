...
- **In-App Updates**: `in_app_update` for Google Play Store updates on Android is wrapped by `lib/logic/in_app_update_service.dart`
...
- `lib/logic/`: Core game mechanics. Includes `chess_board.dart`, `chess_game.dart`, `game_controller.dart`, `chess_piece.dart`, `chess_constants.dart`, `shared_functions.dart`, `move_calculation/`, `timer_service.dart`, `audio_service.dart`, `haptic_service.dart`, `ad_service.dart`, `in_app_update_service.dart`, `game_state_storage.dart`, `play_games_service.dart`, and Flame Sprite logic (`chess_piece_sprite.dart`).
...
17. **Game Save & Resume**: `GameStateStorage` persists the full game state (player settings, move history, timer values, `timerMode`, and undo bank) to SharedPreferences. `saveGameState()` (debounced 400ms) is called after each move; `saveGameStateImmediate()` is called on app lifecycle pause/inactive. `restoreGameState()` in `AppModel` replays the saved moves to reconstruct board state. The `ChessView` passes `isResuming: true` when navigating to a resumed game, which triggers `restoreGameState()` instead of `newGame()`.
18. **Time Control Customizations**: Supports two main timing modes when an increment is active: `increment` (Fischer style, where increment is added after the turn) and `delay` (Simple delay/USCF style, where the player's clock is held for the delay duration before decrementing). The active delay time is displayed visually on the player's timer widget.
19. **In-App Update Lifecycle**: `InAppUpdateService` is called asynchronously on startup in `main()` to check for update availability. It will trigger a flexible or immediate update dialog using Google Play API. Errors are caught silently to prevent sideloaded/debug build failures.
20. **JVM Target & Kotlin Compatibility**: Ensure JVM target compatibility remains aligned at version `17` for both Java and Kotlin compilation tasks across the main app and all subproject dependencies in `android/build.gradle`.
21. **R8 Minification & Native Library Compression**: The release build type uses minifyEnabled, shrinkResources, and `proguard-rules.pro` to ignore warnings for missing Play Core classes (`-dontwarn com.google.android.play.core.**`) and keep Stockfish wrapper classes (`-keep class **.*stockfish*.** { *; }`). Native `.so` libraries must be compressed using `useLegacyPackaging = true` to optimize the final APK size.
22. **Stockfish Castling Move Translation**: The native Stockfish engine expects standard UCI castling notation (e.g. `e1g1`, `e1c1`), whereas the board logic internally models castling as a King "capturing" its Rook (`Move(60, 63)`). Translate castling moves in `StockfishService.msoToUCI` and `_uciToMove` to prevent sprite desyncs or engine crashes.
23. **Board Size & Notation Styling**: The chess board has a flat, edge-to-edge layout without rounded corners or shadow overlays. Algebraic coordinates in `_NotationOverlay` use the alternating tile colors (`lightTileColor` text on dark tiles, and `darkTileColor` text on light tiles) for optimal contrast and readability.
