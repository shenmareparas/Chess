...
9. **Stockfish Lifecycle**: Initialize on demand, dispose in `AppModel.dispose()`. Perform the UCI handshake (`uci` + `isready` -> `readyok`) exactly once per engine instance using a shared `_uciReadyCompleter`. Use `CancelableOperation` to cancel in-flight AI requests on game reset. Once the handshake completes, configure the engine with `Use NNUE` = false (prevents C++ SIGSEGVs), `Hash` = 16 (prevents OOMs), and `Threads` = 1 (prevents native lock contention ANRs). Filter the `stdout` listener to only log key lines (`readyok`, `bestmove`) under `kDebugMode` to avoid flooding the method channel.
