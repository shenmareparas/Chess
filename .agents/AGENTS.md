...
4. **Piece Themes & Preview**: Register new themes in `PIECE_THEMES` in `lib/model/user_preferences.dart`, add assets under `assets/images/pieces/<formatted_name>/`, and update `pubspec.yaml`. `main.dart` preloads only the active and Classic themes asynchronously after `runApp()` to protect main-thread RAM and prevent `OutOfMemoryError` failures. Non-active piece themes load lazily on-demand when selected or previewed. Settings screen previews **must** use the lightweight `PiecePreview` StatelessWidget — never a Flame `GameWidget`.
5. **Settings UI**: App and piece theme pickers are cached outside reactive builder loops and debounced 150ms. Selected rows use subtle fill, no outline border. Settings toggles: Auto-Rotate Board (2P), Auto-Rotate Pieces (2P) (disabled when Board Rotation is active), Move Hints & Highlights, Show Notation, Allow Undo/Redo, Show Move History, Sound, Haptic Feedback, and an Achievements tile.
...
7. **No Destructive Overrides**: Do not remove core features (Undo/Redo, Timers, Difficulty, game save/resume) unless explicitly requested. Supports 5 difficulty levels mapped as Beginner (400 ELO), Casual (800 ELO), Intermediate (1200 ELO), Advanced (1600 ELO), and Master (2000 ELO). Levels 1 & 2 run Stockfish at Skill 0 with a randomized blunder pass (60% and 25% chance of random legal moves respectively). Levels 3–5 natively leverage `UCI_LimitStrength` and `UCI_Elo` options.
...
9. **Stockfish Lifecycle**: Initialize on demand, dispose in `AppModel.dispose()`. Configure the engine with `Use NNUE` = false (prevents C++ SIGSEGVs), `Hash` = 16 (prevents OOMs), and `Threads` = 1 (prevents native lock contention ANRs) **before** issuing `uci` + `isready` handshakes. Perform the UCI handshake exactly once per engine instance using a shared `_uciReadyCompleter`. Use `CancelableOperation` to cancel in-flight AI requests on game reset. Filter the `stdout` listener to only log key lines (`readyok`, `bestmove`) under `kDebugMode` to avoid flooding the method channel.
...
12. **Ad Reward & Fallback**: Rewarded ads (1 Ad = 1 Undo) must degrade gracefully. Grant the undo even on ad failure or offline. `grantUndoFromAd()` adds to the undo bank; do not auto-execute the undo. Each new game starts with 1 free undo. Match exit ads (`showExitInterstitialAd`) use AdMob Console server-side frequency capping and degrade gracefully without blocking navigation back to `MainMenuView`.
...
16. **Play Games Services**: `PlayGamesService` is Android-only (disabled on iOS). Hook points: `onGameStarted` from `AppModel.newGame`, `onPlayerWon` from `AppModel.endGame`, `onPawnPromotion` from `GameController.promote` (human only), `onCheckDelivered` from `GameController._moveCompletion` (human only). Scheduled in a non-blocking `Future.microtask()` after initial frame render to prevent main-thread IPC binder ANRs. All calls fire-and-forget, errors swallowed silently.
...
19. **Time Controls**: `increment` = Fischer (added after turn ends). `delay` = USCF Simple Delay (clock held for delay duration before decrementing). `TimerService` handles both. Active delay is displayed visually on the active player's `TimerWidget`. Custom timer input is validated to stay within the 1-180 minutes range.
...
22. **R8 Minification & Native Library Compression**: Release builds use `minifyEnabled`, `shrinkResources`, `proguard-rules.pro` (`-dontwarn com.google.android.play.core.**`, `-keep class **.*stockfish*.** { *; }`, `-keep class androidx.startup.** { *; }`, `-keep class androidx.work.** { *; }`, `-keep class androidx.room.** { *; }`, `-keep class xyz.luan.audioplayers.** { *; }`, `-keep class com.google.android.gms.** { *; }`). Native `.so` libs use `useLegacyPackaging = true` and explicit NDK `abiFilters 'armeabi-v7a', 'arm64-v8a', 'x86_64'`. `AndroidManifest.xml` has `android:largeHeap="true"`.
...
25. **Interactive Move History**: `MoveList` moves are tappable. Sets `historyViewIndex` on `AppModel`, rewinds board, pauses AI (timers keep running), disables board input. Navigation row on bottom-right displays left/right chevron buttons (tappable at full 56px height, all in `ExcludeSemantics`). A dedicated glowing, theme-adaptive glass pill **RESUME** button replaces the central `GameStatus` indicator at the top of the screen when history mode is active, preventing board layout shifts. Active tile auto-scrolls to center. First scroll jumps directly, while subsequent selections animate smoothly. 2-second press-and-hold copies all moves to clipboard and shows a 4-second custom glassmorphic `FToast` matching the style of the pawn promotion Easter egg countdown.
26. **Piece Rotation**: In two-player offline mode, pieces can be auto-rotated (rotated by 180 degrees/pi on Player 2's turn) if `Auto-Rotate Pieces (2P)` is enabled and `Auto-Rotate Board (2P)` is disabled.
27. **Checkmate Isolate & Workers**: The heavy `kingInCheckmate` loop runs entirely off the main/UI thread inside a persistent background isolate managed by `CheckmateWorker` (spawned once in `GameController`'s constructor and cleaned up via `dispose()`). Game outcomes track the winner correctly even after post-turn flips by passing `winner` explicitly to `AppModel.endGame()`.
28. **Dialog Styling & Effects**: All in-game confirmation/info dialogs (Leave Game, Restart Game, Reset Settings, Out of Undos, Ad Not Ready) must follow identical design guidelines: a `GlassPanel` (borderRadius 24, padding 20, color `Color(0x80201F1F)`), maximum width 300, matching titles (fontSize 20, bold, Color `0xFFE5E2E1`), body texts (fontSize 14, Color `0xFFC3C8C2`, line height 1.4), and standard action/cancel buttons (height 46, borderRadius 12). Confetti on winning is guarded by `_hasPlayedConfetti` state variable to prevent repeating during history reviews or other widget rebuilds.
